...
final class SupabaseClientProvider {
    ...
    private init() {
        do {
            ...
            self.client = nil
            let msg = (error as? SupabaseConfigError)?.message ?? error.localizedDescription
            print("⚠️ Supabase configuration error: \(msg)")
            #if DEBUG
            print("💡 Use \"Skip for now (Debug)\" on login to continue without Supabase.")
            #endif
        }
    }
